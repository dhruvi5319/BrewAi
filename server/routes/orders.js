...
function validateOrderPayload(body) {
    ...
    const items = b['items'].map((item) => {
        ...
        const customizations = {
            ...
            addons: Array.isArray(cust['addons']) ? cust['addons'] : [],
            specialInstructions,
        };
        return {
...
ordersRouter.post('/', (req, res) => {
    ...
    try {
        let orderId;
        const insertOrder = db.prepare('INSERT INTO orders (subtotal, notes) VALUES (?, ?)');
        const insertOrderItem = db.prepare('INSERT INTO order_items (order_id, menu_item_id, name, unit_price, quantity, customizations_json) VALUES (?, ?, ?, ?, ?, ?)');
        const createOrder = db.transaction(() => {
            const orderResult = insertOrder.run(payload.subtotal, payload.notes);
            orderId = orderResult.lastInsertRowid;
            for (const item of payload.items) {
                insertOrderItem.run(orderId, item.menuItemId, item.name, item.unitPrice, item.quantity, JSON.stringify(item.customizations));
            }
            return orderId;
        });
        orderId = createOrder();
