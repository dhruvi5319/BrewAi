...
# downstream tooling that may read it.
# The actual Vite bind comes from the --host 0.0.0.0 CLI flag in EXEC_CMD below.
...
# === Pre-exec snippet: write vite.config.pivota.ts overlay if needed ===
...
# This project uses concurrently to start both Express (tsx server.ts) and Vite.
# Vite is invoked with --host 0.0.0.0 via the pivota overlay config (react-vite catalog).
EXEC_CMD='npm run dev -- --config vite.config.pivota.ts --host 0.0.0.0 2>/dev/null || npm run dev -- --host 0.0.0.0'
ATTEMPT=1
DELAY=1
while (( ATTEMPT <= 3 )); do
